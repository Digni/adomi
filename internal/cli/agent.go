...
func generateSkillContent() string {
	return strings.Join([]string{
		...
		"To check whether CI passed for a change you just pushed, run `pipeline list --last <N>` and match your branch or commit against each run's `sourceBranch` and `sourceVersion`; a run still showing `inProgress` means CI has not finished, so check again later. Once you know the run ID, use `pipeline get <run-id>` for a cheap single-run re-check.",
