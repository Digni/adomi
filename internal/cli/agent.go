...
func generateSkillContent() string {
	return strings.Join([]string{
		...
		"```",
		"",
		"The command exports wiki Markdown and metadata below `.adomi/context/wikis/` and prints only the exported context directory path to stdout.",
		...
		"## Process",
		"",
		"1. Identify whether the user is asking about a work item, wiki, pull request, PR maintenance, or general Azure DevOps context.",
		"2. Resolve the most likely profile/project from repo config, global config, remotes, and folder path hints.",
		"3. Run the appropriate `adomi ado fetch`, `adomi ado wiki fetch`, `adomi ado pr fetch`, `adomi ado comment`, or conservative PR maintenance command, adding `--profile <profile-name>` when no `.adomi/` folder is present.",
