...
func generateSkillContent() string {
	return strings.Join([]string{
		...
		"",
		"Before replying to or resolving/reopening review threads, fetch and inspect PR context with `adomi ado pr fetch <pull-request-id>` unless the user has already provided the relevant thread details.",
		...
		"Successful PR maintenance commands print only data to stdout: PR ID for `ensure`, comment ID for `reply`, thread ID for `resolve`/`reopen`, or one compact JSON object when `--json` is used. Validation, prompts, diagnostics, and Azure DevOps errors belong on stderr and leave stdout empty.",
