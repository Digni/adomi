...
func generateSkillContent() string {
	return strings.Join([]string{
		...
		"",
		"`adomi` fetches Azure DevOps context into local files so you can inspect requirements, comments, attachments, and pull request discussions before planning or coding. It can also post narrow work item comments when the user asks you to report a result back to Azure DevOps.",
		"",
		"## When to Use",
		"",
		"Use this skill when:",
		...
		"3. Run the appropriate `adomi ado fetch`, `adomi ado pr fetch`, `adomi ado comment`, or conservative PR maintenance command, adding `--profile <profile-name>` when no `.adomi/` folder is present.",
