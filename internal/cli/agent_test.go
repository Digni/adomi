...
func requiredAdomiSkillContent() []string {
	return []string{
		...
		"Fetch progress and summaries are written to stderr",
		"`--json` replaces the default path stdout with one compact JSON object",
		"Authentication confirmations are written to stderr",
		"`action` and `credentialRef`",
		"`path`, `workItems`, and `attachments`",
		"comments/<item-id>.json",
		"Comment progress does not change attachment counts",
		"`path` and `pages`",
		"`path`, `threadCount`, and `commentCount`",
		...
		"Discovery always prints compact JSON",
		"1,000 pages, 100,000 PRs, and 8 MiB per page",
		"adomi ado pr <pull-request-id>",
		"adomi ado pr ensure --title <title>",
		"adomi ado pr ensure --description-file <path>",
		"adomi ado pr link <pull-request-id> --work-item <work-item-id> [--work-item <work-item-id>...]",
		"Repeat `--work-item`",
		"already-linked work items as successful no-ops",
		"buffers stdout until every requested work item succeeds",
		"earlier links remain persisted",
		"code read and work item write permissions",
		"explicit work-item-to-PR linking",
		"does not provide generic relation editing, unlink/list operations, or work item field/state mutation",
		...
		"perform the exact lifecycle or reviewer-vote operation the user explicitly authorized",
		...
		"PR lifecycle and reviewer-vote operations require Azure DevOps credentials with code write permission",
		"fetch and inspect the pull request immediately before the mutation unless equivalent current state was supplied by the user",
		"only when the user explicitly requested that exact governance outcome",
		"Do not infer authorization from a request to create, update, review, or discuss a pull request",
		"Completion flags overlay fetched supported preferences; omitted values remain unchanged",
		"Branch policies remain enforced",
		"Immediate completion is pinned to the source commit from the preflight",
		"Auto-complete may finish immediately",
		"Neither command polls for final merge success",
		"Votes are cast only as the authenticated Azure DevOps user",
		"does not accept an arbitrary reviewer identity or raw vote value",
		"Cancellation applies only while the pull request remains active",
		"fetch and inspect PR context",
		"PR/thread write permissions",
		"arbitrary reviewer management, vote reset, wait-for-author voting, policy bypass, abandoned-PR reactivation, and completed-PR reversion remain unavailable",
		...
		"adomi ado login --profile <profile-name> [--json]",
		...
		"adomi ado pipeline list --branch <branch> --last 10",
		"server-side branch filter on every page",
		"adomi ado pipeline get <run-id> [--profile <profile-name>] [--global]",
		"exact `inProgress` runs",
		"`pipeline list --last <N>` returns the N most recently queued runs in the project across any status, with N from 1 through 200",
		"match your branch or commit against each run's `sourceBranch` and `sourceVersion`",
		...
		"vso.test",
		"128 MiB",
		"No published tests were returned",
